...
setSquareRuler.create = function (containerId, settings) {
    ...
    setSquareRulerInstance.setAttribute('style', `z-index:${settings.zIndex};position:fixed;pointer-events: none`);
    setSquareRulerInstance.id = settings.id;

    let rootContainer = document.getElementById(containerId);
    rootContainer.appendChild(setSquareRulerInstance);

    setSquareRulerSettingsDictionary[settings.id] = settings;

    setSquareRuler.drawSetSquareRuler(settings);
}
...
setSquareRuler.drawSetSquareRuler = function (settings) {
    let id = settings.id;
    let setSquareRulerInstance = document.getElementById(settings.id);
    ...
    if (setSquareRulerInstance) {
        ...
        parentGroup.setAttribute('onpointerdown', 'setSquareRuler.mouseDownSetSquareRuler(event)');
        parentGroup.setAttribute('onpointermove', 'setSquareRuler.mouseMoveSetSquareRuler(event)');
        parentGroup.setAttribute('onpointerup', 'setSquareRuler.mouseUpSetSquareRuler(event)');
        parentGroup.setAttribute('onpointerover', 'setSquareRuler.mouseOverSetSquareRuler(event)');
        parentGroup.setAttribute('onpointerout', 'setSquareRuler.mouseOutSetSquareRuler(event)');
        ...
        settings.rectangleCenter = {
            x: (vertexB.x + vertexC.x) / 2,
            y: (vertexB.y + vertexA.y) / 2
        }
    }
}
...
setSquareRuler.calculateDistanceToRulerRightSide = function (settings, position) {
    ...
    let va_tx = settings.vertexA.x - settings.rectangleCenter.x;
    let va_ty = settings.vertexA.y - settings.rectangleCenter.y;

    let vc_tx = settings.vertexC.x - settings.rectangleCenter.x;
    let vc_ty = settings.vertexC.y - settings.rectangleCenter.y;

    let p_tx = position.x - settings.rectangleCenter.x - settings.dx;
    let p_ty = position.y - settings.rectangleCenter.y - settings.dy;
    ...
    if (settings.angle == -45 || settings.angle == 135 || settings.angle == 315 || settings.angle == -225) {
        return Math.abs(p_ty - va_y);
    }
    else if (settings.angle == 45 || settings.angle == 225 || settings.angle == -135 || settings.angle == -315) {
...
setSquareRuler.calculateDistanceToRulerLeftSide = function (settings, position) {
    ...
    let vb_tx = settings.vertexB.x - settings.rectangleCenter.x;
    let vb_ty = settings.vertexB.y - settings.rectangleCenter.y;
    ...
    let slope = 0;
    let p_slope = 0;
    if (settings.angle == 45 || settings.angle == 225 || settings.angle == -135 || settings.angle == -315) {
        return Math.abs(p_ty - va_y);
    }
    else if (settings.angle == 135 || settings.angle == 315 || settings.angle == -225 || settings.angle == -45) {
        return Math.abs(p_tx - va_x);
    }
    else {
...
setSquareRuler.calculateDistanceToRulerBottomSide = function (settings, position) {
    ...
    let px = position.x - settings.rectangleCenter.x - settings.dx;
    let py = position.y - settings.rectangleCenter.y - settings.dy;

    let height = settings.vertexB.y - settings.rectangleCenter.y;
    ...
    if (settings.angle === 0) {
        ...
        let translatedCurrentPositionX = position.x - settings.rectangleCenter.x - settings.dx;
        let translatedCurrentPositionY = -(position.y - settings.rectangleCenter.y - settings.dy);
...
setSquareRuler.mapDistanceToRulerRightSide = function (settings, position, strokeThickness) {
    ...
    let center = settings.rectangleCenter;

    let va_tx = settings.vertexA.x - center.x ;
    let va_ty = settings.vertexA.y - center.y - (strokeThickness / 2 + rulerShift) / Math.cos(Math.PI / 4);

    let vc_tx = settings.vertexC.x + (strokeThickness / 2 + rulerShift) / Math.cos(Math.PI / 4) - center.x;
    let vc_ty = settings.vertexC.y - center.y ;
    ...
    if (settings.angle == -45 || settings.angle == 135 || settings.angle == 315 || settings.angle == -225) {
        return { x: position.x, y: va_y + settings.rectangleCenter.y + settings.dy };
    }
    else if (settings.angle == 45 || settings.angle == 225 || settings.angle == -135 || settings.angle == -315) {
        return { x: va_x + settings.rectangleCenter.x + settings.dx, y: position.y };
    }
    else {
...
setSquareRuler.mapDistanceToRulerLeftSide = function (settings, position, strokeThickness) {
    ...
    let vb_tx = settings.vertexB.x - (strokeThickness / 2 + rulerShift) / Math.cos(Math.PI / 4) - center.x;
    let vb_ty = settings.vertexB.y - center.y ;
...
setSquareRuler.mapDistanceToRulerBottomSide = function (settings, position, strokeThickness) {
    let height = settings.y - settings.rectangleCenter.y;
    if (settings.angle === 0) {
    ...
    if (settings.angle === 90 || settings.angle === -270) {
        ...
        let x = (p / Math.sin(beta) - b) / (s + 1 / Math.tan(beta));
        let y = Math.tan(beta) * x + b;
        return { x: settings.rectangleCenter.x + settings.dx + x, y: settings.rectangleCenter.y + settings.dy - y };
